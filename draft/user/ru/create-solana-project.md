...
>
> Показать вкладку `Triggers` в Solana project до создания triggers, с видимыми действиями `Add trigger` и `Import triggers`.
...
>
> Показать шаг `Source` в мастере импорта с открытым списком `Source network` и выбранным Solana source.
...
>
> Показать шаг `Generate` с заполненными `Category`, `Program ID`, загруженным IDL JSON и кнопкой `Generate triggers from IDL`.
...
>
> Показать таблицу review с колонками `Trigger`, `Type`, `Description`, несколькими Solana trigger candidates и отмеченными checkboxes.
...
>
> Показать один сгенерированный Solana trigger в trigger wizard, желательно на шаге настройки event/call или текста уведомления.
