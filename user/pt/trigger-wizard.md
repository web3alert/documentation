...
A source escolhida determina o ramo seguinte de configuração: EVM, Substrate ou Solana.
...
Escolha obrigatória do tipo de Solana item: `event` ou `call`.

`event` corresponde a um program event decodificado a partir de logs. `call` corresponde a uma Solana instruction do program escolhido.

#### Program ID

Public key obrigatório do Solana program.

#### IDL

JSON IDL do program escolhido. O wizard pode tentar carregá-lo automaticamente a partir de Anchor IDL account ou Program Metadata. Se não puder ser carregado, deve ser colado manualmente. Sem IDL não é possível criar um trigger Solana event/call fiável.

#### Event / Call

Entry obrigatória a partir do IDL. Para `event` escolhe-se uma entrada de `events`; para `call` escolhe-se uma instruction de `instructions`. As accounts de uma instruction ficam disponíveis como `source.accounts.*`.

### Types Source

`Types source` é uma configuração opcional ao nível do trigger. Indica ao wizard de onde deve carregar o catálogo de tipos usado por campos de schema com tipo `lookup`.

Campos lookup não incorporam todo o schema aninhado dentro do campo. Em vez disso, guardam uma referência a um tipo com nome dentro de um catálogo. Quando o subscription wizard ou o trigger editor precisa de mostrar os campos internos desse tipo lookup, pede o catálogo para o trigger escolhido e resolve a referência. Isto é útil para Substrate runtime metadata, Solana IDL custom types, catálogos importados e outros schemas em que os objetos são grandes, reutilizados em vários sítios ou recursivos. Mantê-los como lookups evita copiar estruturas profundas em cada campo e evita que tipos recursivos se expandam indefinidamente na UI.

Se `Types source` estiver desligado, é usado o comportamento por defeito: o backend tenta primeiro usar types específicos do trigger, se existirem, e depois faz fallback para project/source types quando podem ser inferidos a partir do source escolhido. Timer triggers normalmente não têm source inferido, por isso o catálogo fica vazio a menos que esta configuração esteja ativa.

Modos disponíveis:

- `Source node` - escolher um data source e usar o catálogo de tipos importado do runtime ou metadata desse source.
- `API / imported catalog` - chamar uma HTTP API que devolve um catálogo de tipos. O wizard pede um único Base URL. O backend carrega o catálogo a partir de `Base URL + /catalog`; esse endpoint pode devolver uma lista completa de types, um objeto `{ schemas: ... }`, um wrapper `{ types: [...] }`/`{ data: [...] }` ou uma lista importada de itens `{ id, schema }`.
...
Para controlos de filtro dinâmicos como `cascade`, um API catalog também deve fornecer `Base URL + /lookup`. O wizard chama esse endpoint com query params como `ref`, `q`, `value`, `limit` e IDs dos passos cascade anteriores para carregar listas de options para os `Lookup ref` selecionados. A condition guardada na subscription continua a armazenar um único valor de filtro normal.
...
Polling de readiness opcional e não bloqueante. Use quando uma API externa indexa dados com atraso: o evento on-chain já chegou, mas os metadados aparecem na API apenas alguns segundos depois. Sem retry o provider retorna uma resposta vazia ou incompleta, o transform aplica valores de fallback e os filtros de subscription sobre os output fields não correspondem.

Quando a resposta do provider "não está pronta", a execução é estacionada e repetida automaticamente mais tarde. O worker não fica bloqueado durante a espera, o evento mantém sua identidade original e o usuário recebe no máximo uma notificação. O retry é executado antes do transform e antes dos filtros de subscription, então os filtros veem os dados enriquecidos. Um trigger aguardando retry não é considerado quebrado.

Campos:

- `Enabled` - habilita o retry para o provider;
- `Attempts` - número máximo de tentativas, de 1 a 20;
- `Delay, ms` - pausa base entre tentativas, de 250 a 60000;
- `Backoff` - como a pausa cresce: `Fixed` - a mesma pausa sempre, `Linear` - pausa × número da tentativa, `Exponential` - a pausa dobra a cada tentativa;
- `When exhausted` - o que fazer quando as tentativas se esgotam:
  - `Continue with last response` - continuar com a última resposta como está; o transform aplica seu próprio fallback;
  - `Fail the provider` - tratar o provider como falho: um provider `Optional` produz resultado vazio, um obrigatório termina a execução sem output;
- `Retry when` - condições que marcam a resposta como "não pronta":
  - `Empty array` - a resposta é um array vazio;
  - `Missing path` - não há valor em `Ready when path`;
  - `HTTP error` - a request falhou com erro de rede ou status não-2xx; sem esta flag tais erros continuam sendo erros normais do provider;
- `Ready when path` - caminho para um campo da resposta que deve estar presente; os segmentos são separados por pontos, índices de array são números;
- `Equals` - valor opcional ao qual o campo em `Ready when path` deve ser igual; suporta template substitutions, strings hex são comparadas sem diferenciar maiúsculas.

O teste do provider executa uma única tentativa e não espera readiness - o polling acontece apenas no runtime.

Exemplo de configuração via API/MCP:
...
Via API/MCP também são aceitos `maxDelayMs` (teto de uma única pausa) e `maxElapsedMs` (orçamento total de espera); eles não aparecem na UI.
...
O provider GraphQL suporta a retry policy. Os campos e o comportamento estão descritos em [HTTP -> Retry until ready](#retry-until-ready).
...
Tipo de leitura: `EVM contract`, `Substrate storage` ou `Solana account`.

#### EVM Contract <Badge type="tip" text="state type" />

Executa um read call view/pure de um método do contrato EVM.
...
Lê uma storage entry do Substrate runtime.
...
Lê uma Solana account e decodifica seu conteúdo.

##### Source

Solana source. Por defeito é usado source do trigger.

##### Account

Public key da account cujo estado deve ser lido. Suporta templates, por exemplo `{{ source.accounts.base_mint }}`.

##### IDL

IDL JSON opcional com account definitions. Se estiver ativo, o provider decodifica estritamente com esse IDL. Se estiver desativado, tenta resolver a schema automaticamente por `jsonParsed`, built-in layouts como SPL Token/Metaplex Metadata, ou IDL automático por Anchor/Program Metadata.

O resultado fica disponível como `providers.<id>` e inclui campos base de account e `data` decodificado quando uma schema é encontrada. Resultados corretos ficam em cache por pouco tempo para evitar RPC requests repetidos ao mesmo account.
...
O provider RPC suporta a retry policy. Os campos e o comportamento estão descritos em [HTTP -> Retry until ready](#retry-until-ready).
...
O passo começa com `Output mode`.

No modo `Single output`, o passo contém dois painéis JavaScript:
...
No modo `Multi-output`, o passo contém um painel JavaScript, `Output items transform`. É usado quando um source tick ou provider query pode produzir vários alert items independentes. O transform deve devolver:

```js
{
  outputs: [
    {
      raw: { /* campos correspondentes a Raw output */ },
      human: { /* campos correspondentes a Human output */ },
    },
  ],
}
```

Cada item em `outputs` é emitido como um trigger output separado. As schemas `Raw output` e `Human output` descrevem um item emitido, e todos os items são validados por essas schemas.
...
Periodicidade de entrega recomendada para novas subscriptions criadas a partir deste trigger.

- `Every match` envia cada output correspondente.
- `Once` envia apenas o primeiro output correspondente para uma regra de subscription.
- `Once per key` envia o primeiro output para cada key path selecionado. Para triggers fan-out ou polling, usa um output field estável, como o id da entidade.

Se este painel estiver desativado, as novas subscriptions começam com `Every match`.
