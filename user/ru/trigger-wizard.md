...
Для динамических filter controls вроде `cascade` API catalog также может предоставлять lookup endpoint. Wizard вызывает его, чтобы загрузить списки options для выбранных `Lookup ref`, включая зависимые списки вроде series -> event -> market. Сохраненная condition подписки по-прежнему хранит одно обычное значение фильтра.
