...
`Data sources` - это источники blockchain/runtime данных, из которых Web3alert получает blocks, transactions, events, extrinsics, Solana instructions/calls и metadata.
...
Сейчас wizard поддерживает три типа custom data sources.
...
Solana source используется для Solana-compatible сетей и RPC endpoints.

Он подходит для Solana blocks, successful instructions/calls и program events, которые декодируются по IDL. Для Solana source важно, чтобы у программы был доступен IDL: wizard может попытаться загрузить его автоматически через Anchor IDL account или Program Metadata, но если это не получается, IDL JSON нужно вставить вручную.

Без IDL Web3alert не создает Solana event/call triggers, потому что невозможно надежно построить структуру аргументов, accounts и output schema.

Для Solana source обычно используется HTTP RPC URL. Source читает каждый slot/block через RPC, а matching events/calls извлекает из полного блока без отдельных `getTransaction` requests.

## Список data sources

Раздел `Data Sources` показывает таблицу источников.

Источники группируются по типу или runtime category, например `EVM`, `Substrate`, `Solana` или другой plugin/runtime type.
...
Сейчас доступны `EVM`, `Substrate` и `Solana`.
...
Для EVM и Solana обычно используется HTTP RPC URL. Для Substrate обычно используется WebSocket URL.
...
Для EVM и Solana sources этот шаг обычно ничего не требует: они не используют signed extensions, custom runtime types или RPC extension definitions в этом flow.
