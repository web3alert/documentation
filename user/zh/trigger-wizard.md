...
- `API / imported catalog` - 调用返回类型目录的 HTTP API。Wizard 只需要一个 Base URL。Backend 会从 `Base URL + /catalog` 加载目录；该 endpoint 可以返回完整 types 列表、`{ schemas: ... }` 对象、`{ types: [...] }`/`{ data: [...] }` 包装对象，或导入的 `{ id, schema }` item 列表。
- `Inline / manual` - 粘贴一个 JSON object，其中 keys 是类型名称，values 是类似 JSON Schema 的类型定义。

对于 `cascade` 这样的动态 filter control，API catalog 也应该提供 `Base URL + /lookup`。Wizard 会用 `ref`、`q`、`value`、`limit` 以及前面 cascade 步骤的 ID 等 query params 调用该 endpoint，为所选 `Lookup ref` 加载 option 列表。保存后的 subscription condition 仍然只保存一个普通 filter value。
