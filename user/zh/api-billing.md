...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `code` | 是 | 优惠券代码。删除首尾空白（`trim`），并进行不区分大小写的匹配（`case-insensitive`）。 |

优惠券对应的付费账户套餐周期只会应用一次。系统会根据当前付费周期延长
现有周期、叠加适用周期，或开始一个新周期。

| 兑换场景 | 必须行为 |
| --- | --- |
| `code-normalization` | 应用 `trim` 和 `case-insensitive` 匹配。 |
| `same-account-retry` | 返回 `same-couponId`、`same-subscriptionId` 和 `same-invoiceId`，并保证 `no-second-application`。 |
| `unavailable-or-unsafe` | 优惠券若为 `redeemed-by-another-account`、`deleted`、`invalid`，或存在 `unsafe-subscription-lineage`，必须 `fail-closed`。 |
| `paid-account-plan-period` | 根据情况执行 `extend`、`overlay` 或开始 `new-term`，即 `as-applicable`。 |

响应：HTTP 200 OK。

| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `couponId` | 是 | 已兑换优惠券的标识符。 |
| `subscriptionId` | 是 | 兑换后账户套餐订阅的标识符。 |
| `invoiceId` | 是 | 优惠券已支付账单的标识符。 |
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `planId` | 是 | 付费账户套餐：`advanced` 或 `pro`。 |
| `durationMonths` | 是 | 优惠券有效期：`1`、`3`、`6` 或 `12` 个月。 |
| `requestId` | 否 | 由 8-128 个 RFC 3986 unreserved 字符组成的幂等键：`A-Z`、`a-z`、`0-9`、`.`、`_`、`~` 和 `-`。为兼容旧客户端可省略，但建议提供。 |

同一账户内，一次经用户确认的礼品购买意图应使用一个稳定的 `requestId`。
如果发生 timeout 或其他结果未知的情况，请使用相同的套餐和有效期重试。

| 购买场景 | 必须行为 |
| --- | --- |
| `same-intent` | 使用 `same-requestId`、`same-planId` 和 `same-durationMonths`。 |
| `exact-replay` | 返回 `same-couponId` 和 `same-code`，并保证 `no-second-debit` 和 `no-second-referral-reward`。 |
| `conflicting-payload` | 使用相同请求标识符但更改套餐或有效期的请求会被 `rejected`。 |
| `new-intent` | 生成 `new-requestId`。 |
| `missing-requestId` | 保持 `backward-compatible`，但为 `not-retry-safe` 且 `not-idempotent-across-HTTP-attempts`。 |
