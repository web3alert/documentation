...
`Import triggers` 是一个 wizard，用于根据指定 configs 或 metadata 快速生成一组 triggers，例如根据 EVM 合约 ABI、Substrate pallet metadata 或 Solana program IDL。
...
对于 Solana source，需要填写 `Category`、`Program ID`、`IDL` 和 `Source item`。

IDL 可以从 Anchor IDL account 或 Program Metadata 自动加载。如果无法自动加载，需要手动粘贴 IDL JSON。没有 IDL 时不会生成 Solana triggers，因为无法可靠解码 events、instructions、arguments 和 accounts。

Solana import 支持 `Event` 和 `Call`。`Call` 对应 Solana instruction，但 UI 保留通用术语 `Call`。
