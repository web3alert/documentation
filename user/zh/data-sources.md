...
`Data sources` 是 blockchain/runtime 数据来源，Web3alert 会从中接收 blocks、transactions、events、extrinsics、Solana instructions/calls 和 metadata。
...
目前 wizard 支持三种 custom data source 类型。
...
Solana source 用于兼容 Solana 的 networks 和 RPC endpoints。

它适合 Solana blocks、成功的 instructions/calls，以及通过 IDL 解码的 program events。Wizard 可以尝试从 Anchor IDL account 或 Program Metadata 自动加载 IDL；如果自动加载失败，需要手动粘贴 IDL JSON。

没有 IDL 时，Web3alert 不会创建 Solana event/call triggers，因为无法可靠构建 arguments、accounts 和 output schema 结构。

Solana source 通常使用 HTTP RPC URL。

## Data sources 列表

`Data Sources` section 显示 sources 表格。

Sources 会按 type 或 runtime category 分组，例如 `EVM`、`Substrate`、`Solana` 或其他 plugin/runtime type。
...
EVM 和 Solana 通常使用 HTTP RPC URL。Substrate 通常使用 WebSocket URL。
...
对于 EVM 和 Solana sources，这一步通常不需要任何内容：在这个 flow 中，它们不使用 signed extensions、custom runtime types 或 RPC extension definitions。
