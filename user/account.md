...
По этому token пользователь получает доступ к Web3alert [API]() и [MCP servers](). Уровень доступных API/MCP возможностей зависит от текущего тарифа account.
...
Вкладка `Billing` управляет балансом, тарифом, billing profile, оплатами и coupons account.
...
В dialog нужно указать amount в EUR. Перед checkout сервис проверяет billing email и country. Если страна не поддерживается платежным провайдером, checkout не начнется.
...
Платежные данные account.

### Billing email

Email для billing checkout и платежных операций.

Перед пополнением баланса или оплатой сервис может потребовать заполнить billing email.

### Country

Страна billing profile.

Country выбирается из списка поддерживаемых стран. Если страна уже сохранена в profile, она отображается в поле. Если страна еще не выбрана, поле показывает `Select country`.
